        ...
        <div className="flex justify-between items-center h-16">
          ...
          <Link to='/' className="flex items-center space-x-2">
            <div className={`p-2 rounded-full ${
              isScrolled ? 'bg-gradient-to-r from-red-600 to-red-700 text-white' : 'bg-white text-red-600'
            }`}>
          ...
          <div className="hidden md:flex items-center space-x-4">
            {/* Search */}
            {/* <div className={`relative ${isScrolled ? 'text-gray-600' : 'text-white'}`}>
              <FaSearch className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2" />
              <input
                type="text"
                placeholder="Search..."
                className={`pl-10 pr-4 py-2 rounded-full text-sm border outline-none transition-all duration-200 ${
                  isScrolled
                    ? 'border-gray-300 bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500'
                    : 'border-red-400 bg-red-800/30 placeholder-red-200 focus:border-white focus:ring-1 focus:ring-white'
                }`}
              />
            </div> */}
            ...
            {/* Profile */}
           {
            user ?
            // logout btn
             <button onClick={handleLogOut} className={`flex items-center space-x-2 px-4 py-2 rounded-full shadow-md transition-all duration-200 ${
              isScrolled
            ...
            <>
            {/* Login btn */}
            <Link to='/login' className={`flex items-center space-x-2 px-4 py-2 rounded-full shadow-md transition-all duration-200 ${
              isScrolled
                ? 'bg-gradient-to-r from-red-600 to-red-700 text-white hover:from-red-700 hover:to-red-800'
                : 'bg-white text-red-600 hover:bg-red-50'
            }`}>
            ...
            {/* register btn */}
            <Link to='/register' className={`flex items-center space-x-2 px-4 py-2 rounded-full shadow-md transition-all duration-200 ${
              isScrolled
                ? 'bg-gradient-to-r from-red-600 to-red-700 text-white hover:from-red-700 hover:to-red-800'
                : 'bg-white text-red-600 hover:bg-red-50'
            }`}>
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className={`px-2 pt-2 pb-3 space-y-1 rounded-lg mt-2 ${
              isScrolled ? 'bg-white border border-gray-200' : 'bg-red-800/90 backdrop-blur-md'
            }`}>
              {navigationItems.map(({ name, href, icon: Icon }) => {
                const isActive = location.pathname === href;
                return (
